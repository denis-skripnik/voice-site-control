const key = 'c82fbac1-cb8d-4729-870a-1644b2c75d4e';
...
class Voice {
    ...
    constructor(key, commands) {
        ...
        window.ya.speechkit.settings.model = 'queries';
        window.ya.speechkit.settings.apikey = key;
    }
    ...
    start() {
        ...
        this.streamer.start({
            ...
            utteranceSilence: 50,
            //приходят только окончательные ответы с `utterance` === true
    ...
    execute(text) {
        ...
        const o = this.commands.find(o => o.validate(command));
...
const commands =  [
    ...
    },
    {
        ...
        execute() {
            ...
            const windowHeight = document.documentElement.clientHeight ||
                window.innerHeight;
            const scrollHeight = Math.floor(0.9 * windowHeight);
            const doScroll = time => {
    ...
    },
    {
        name: 'forward',
        validate(command){
            return command.includes('вперед')
        },
        execute() {
            history.forward();
        },
    ...
    },
    {
        name: 'reload',
        validate(command) {
            return command.includes('обновить')
        },
        execute() {
    ...
    },
    {
        ...
        validate(command) {
            if (this.data.anchor) {
                if (/да|перейти|ок/.test(command)) {
                    this.data.action = `go`;
                    return true;
                } else if (/нет|отмена/.test(command)) {
                ...
                if (/ссылка/.test(command)) {
                    this.data.action = `paint`;
                    return true
                }
                return false;
            }
        ...
        execute(command) {
            ...
            if (action === `go`) {
                ...
                this.data.anchor.style.color = this.data.style.color;
                this.data.anchor.style.background = this.data.style.background;
                this.data.anchor = null;
            } else if (action === `paint`) {
                const c = command.replace('ссылка', '');
                const worlds = c.match(/[а-яА-Я]+/gi);
                ...
                const anchors = Array.from(
                    ...
                    return (
                        ...
                        //содержит слово
                        anchor.innerText.toLowerCase().includes(word)
                    )
                });
                ...
                //фиксируем текущие стили
                const style = getComputedStyle(anchor);
                this.data.style.background = style.background;
                this.data.style.color = style.color;
        ...
        data: {
            ...
            //действие для execute - `go`, `cancel`, `paint`
            action: null
        },
    },
    {
        name: 'help',
        validate(command) {
            return /помощь|справка/.test(command)
        },
        execute() {
            let helpDecor = document.getElementById('__voice_help_decor');
            if (helpDecor) {
                helpDecor.style.display = 'flex';
                return;
            }
            helpDecor = document.createElement('div');
            helpDecor.setAttribute('id', '__voice_help_decor');

            const helpEl = document.createElement('div');

            helpDecor.append(helpEl);
            document.body.append(helpDecor);

            this.setData(helpEl);
        },
        setData(el) {
            const tableHelp = document.createElement('table');
            tableHelp.id = '__voice_help_table';

            tableHelp.innerHTML = '<caption>Действия Voice</caption>' +
                '<thead>' +
                '<tr>' +
                '<th>Название</th>' +
                '<th>Список команд</th>' +
                ...
                '</tr>' +
                '</thead>' +
                '<tbody><tr class = \'__voice_empty\'>' +
                '<td style = \'border: none\' colspan = \'3\'></td>' +
                '</tr></tbody>';

            this.data.forEach((list, i) => {
                list.forEach(el => {
                    tableHelp.children[2].innerHTML +=
                        `<tr><td>${el.name}</td><td>${el.commands.join(', ')}</td><td>${el.description}</td></tr>`
                });
                if (i !== list.length + 1) {
                    tableHelp.children[2].innerHTML +=
                        '<tr class = \'__voice_empty\'>' +
                        '<td style = \'border: none\' colspan = \'3\'></td></tr>';
                }
            });
        ...
        data: [
            [
                {
                    name: 'help',
                    commands: ['помощь', 'справка'],
                    description: 'Открывает список команд',
                },
                {
                    name: 'thank',
                    commands: ['спасибо'],
                    description: 'Закрывает список команд',
                },
            ],
            [
                {
                    name: 'down',
                    commands: ['вниз', 'ниже'],
                    description: 'Прокручивает страницу на один экран вниз',
                },
                {
                    name: 'end',
                    commands: ['конец'],
                    description: 'Перемещает к концу страницы',
                },
                {
                    name: 'up',
                    commands: ['вверх', 'выше'],
                    description: 'Прокручивает страницу на один экран вверх',
                },
                {
                    name: 'beginning',
                    commands: ['начало'],
                    description: 'Перемещает на начало страницы',
                },
            ],
            [
                {
                    name: 'forward',
                    commands: ['вперед'],
                    description: 'Открывает следующую страницу (если она есть)',
                },
                {
                    name: 'back',
                    commands: ['назад'],
                    description: 'Открывает предыдущую страницу (если она есть)',
                },
                {
                    name: 'reload',
                    commands: ['обновить'],
            ...
            [
                {
                    name: 'anchor',
                    commands: ['ссылка + слово из названия'],
                    description: 'Выбирает ссылку на экране и подсвечивает её',
                },
                {
                    name: 'yes',
                    commands: ['да', 'перейти', 'ок'],
                    description: 'Подтвержает переход',
                },
                {
                    name: 'no',
                    commands: ['нет', 'отмена'],
                    description: 'Отменяет переход',
                },
            ],
        ],
    },
    {
        name: 'thank',
        validate(command) {
            return /спасибо/.test(command)
        },
        execute() {
            const helpDecor = document.getElementById('__voice_help_decor');
            if (helpDecor) helpDecor.style.display = 'none';
        },
    },
];
...
const voice = new Voice(key, commands);
voice.create();
voice.start();
